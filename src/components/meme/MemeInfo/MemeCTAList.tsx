...
export const MemeCTAList = ({ id }: Props) => {
  ...
  const {
    collectionInfo: { done },
    onUpdateCollection,
  } = useCollection({ memeId: Number(id) });
    ...
    <div className="flex w-full gap-10 py-40">
      ...
      <Button
        className={`flex h-52 w-full items-center gap-8 rounded-10 ${
          done ? "bg-gray-300" : "bg-gray-900"
        }`}
        onClick={isLogin ? onUpdateCollection : signUpModalProps.onOpen}
      >
        <Icon
          className={`${done ? "[&_*]:fill-gray-800" : "[&_*]:fill-secondary-1000"}`}
          name="collection"
        />
        <span className={`font-suit text-16-semibold-140 ${done ? "text-gray-800" : "text-white"}`}>
          콜렉션에 저장
        </span>
      </Button>
